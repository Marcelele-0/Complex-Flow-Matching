...
generation needs distributional metrics, and :mod:`cyfm.evaluate` reports these at
...
def phase_lag_one(fields: torch.Tensor, keep_fraction: float | None = None) -> float:
    ...
    **The support must be matched between the two batches, or the comparison is biased.**
    ...
    if keep_fraction is None:
        ...
        if flat.numel() > _MAX_SLICED_COEFFICIENTS:
            flat = subsample(flat, _MAX_SLICED_COEFFICIENTS, torch.Generator().manual_seed(0))
