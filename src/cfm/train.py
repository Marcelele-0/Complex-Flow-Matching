...
def main(cfg: DictConfig) -> None:
    ...
    for epoch in range(epochs):
        ...
        if use_wandb:
            ...
            if (epoch + 1) % 10 == 0:
                gt_amp_img = x_1[0, 0].detach().cpu().numpy() # Extract the first channel (Amplitude) of the first batch image
                log_dict["ground_truth_sample"] = wandb.Image(gt_amp_img, caption=f"Epoch {epoch+1} Target Amp")
