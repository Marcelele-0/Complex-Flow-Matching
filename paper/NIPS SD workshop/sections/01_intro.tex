...
\begin{itemize}
    \item \textbf{Mathematical Foundations for Learning Dynamics:} We demonstrate how the geometry of the state space shapes probability paths, analytically proving that flat Euclidean paths suffer from non-physical destructive interference ($1 - \mathbb{E}[\cos(\Delta\theta/2)] \approx 36.3\%$ mid-point attenuation), whereas our paths follow intrinsic cylindrical geodesics.
    \item \textbf{Structure-Preserving Dynamics:} We show how continuous trigonometric embeddings and intrinsic velocity regression globally bound angular velocity ($|u_\theta| \le \pi$), guaranteeing numerical stability for continuous ODE/SDE solvers (e.g., 2nd-order Heun).
    \item We demonstrate the practical value of this geometry-aware prior in a downstream reconstruction experiment using simulated Cartesian $4\times$ $k$-space undersampling.
\end{itemize}
