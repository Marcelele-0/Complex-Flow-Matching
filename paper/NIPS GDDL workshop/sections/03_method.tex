...
To construct a topology-aware generative model for complex-valued MRI data, we formulate continuous normalizing flows directly on the intrinsic manifold of the signal \citep{lipman2023flow, chen2024flow}. Each complex pixel $z = A e^{i\theta} \in \mathbb{C}$ naturally resides on a cylindrical manifold $\mathcal{M} = \mathbb{R}^+ \times S^1$, equipped with the product metric $g = dA^2 + d\theta^2$, such that a 2D complex image is modeled on $\mathcal{M}^{H \times W}$. In contrast, the Euclidean baseline naively operates on 2-channel Cartesian inputs $(\mathrm{Re}(z), \mathrm{Im}(z)) \in \mathbb{R}^2$, regressing straight Euclidean paths $z_t = (1-t)z_0 + t z_1$ toward target velocity $u_{\mathrm{euc}} = z_1 - z_0$ with an unweighted $L_1$ objective $\mathcal{L}_{\mathrm{euc}}(\phi) = \mathbb{E}[|v_x - u_x| + |v_y - u_y|]$. Our cylindrical formulation explicitly decouples amplitude and phase transport: geodesics interpolate each component independently along the shortest angular path on $S^1$, preventing the amplitude collapse inherent to Euclidean interpolation where antipodal trajectories $(\Delta\theta = \pi)$ are forced through the origin ($A \to 0$). Furthermore, this avoids the singular polar metric $g_{\mathrm{polar}} = dA^2 + A^2 d\theta^2 \cong \mathbb{C} \setminus \{0\}$, whose straight Euclidean geodesics also collapse near the removed origin.
