...
\begin{itemize}
    ...
    \item \textbf{Few-step generation across domains.} On synthetic fields ($16\times16$ to $64\times64$) and real speech STFT ($64\times64$), CyFM with the joint coupling reaches a lower error than the best Cartesian baseline at every $k \le 8$ integration steps, with all five seeds separated and no distillation. On raw complex fastMRI knee wavefields ($64\times64$) the pooled advantage is confined to a single step, where it is $1.8\times$ with separated seeds, and reappears from $k=4$ on measures of spatial structure. At a single step the reduction against the best Cartesian arm is $1.8\times$--$3.0\times$ across the three domains, and against a score-based baseline at matched cost it is three orders of magnitude. Run to convergence, we detect no significant difference between the two geometries.
