...
Generative results in this paper are on synthetic complex fields and on real speech spectrograms; the knee MRI cohort enters only through the analytical bridges of Table~\ref{tab:bridge_ablation}, where no network is involved. The next steps are: unconditional generation of complex MRI images (fastMRI, \citealp{zbontar2018fastmri}); adding spatial metrics alongside the pooled sliced $W_2$; and investigating the Cartesian geometry's lower phase error at intermediate step counts.
