...
The fundamental group of a product is the product of the factors', and each factor is homotopy equivalent to $S^1$, so $\pi_1(\mathcal{M}^{H \times W}) = \mathbb{Z}^{HW}$: every pixel wraps independently of the others. This product structure has three consequences:
\begin{itemize}
    \item \textbf{Point-wise Separability:} The product Riemannian metric $g = \sum_{u, v} (dA_{u,v}^2 + d\theta_{u,v}^2)$ decomposes completely point-wise. Geodesics, exponential maps, and logarithmic maps require zero spatial inter-pixel coupling in the probability paths, making them embarrassingly parallel.
    \item \textbf{Vanishing Curvature:} Because each local factor is intrinsically flat (Gaussian curvature $K=0$), the entire product manifold $\mathcal{M}^{H \times W}$ has identically vanishing Riemann curvature ($R \equiv 0$), so the construction carries no curvature-dependent local terms. This is not an advantage over the Cartesian baseline, which is flat as well, and flatness removes neither the $2\pi$ wrap, nor the cut locus at antipodal phase, nor the boundary at $A = 0$.
    \item \textbf{Architectural Synergy:} Spatial correlations are learned by the neural network backbone, while the geometric probability path bounds the target angular velocity ($|u_\theta| \le \pi$) at every spatial coordinate.
\end{itemize}
...
For any pair of configurations $(x_0, x_1) \in \mathcal{M} \times \mathcal{M}$, the target angular velocity under Cylindrical Flow Matching is globally bounded:
\begin{equation}
    |u_\theta| \le \pi.
\end{equation}
In contrast, wherever $z_t \ne 0$ the induced angular velocity under Cartesian flow $z_t = (1-t)z_0 + t z_1$ is:
\begin{equation}
