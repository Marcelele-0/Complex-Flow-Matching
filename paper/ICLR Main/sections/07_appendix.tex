...
Corollary~\ref{cor:pareto_tail} fixes the amplitudes. When they are random as well, a second route to a small numerator opens, since $c$ also vanishes as either amplitude does, and the tail is heavier than index $1$ rather than lighter. The index near $1$ measured in Table~\ref{tab:bridge_ablation} is therefore consistent with this analysis but is not derived from it. The cylindrical bridge admits no such tail: $\theta_t = \theta_0 + t u_\theta$ turns at the constant rate $u_\theta$, and $|u_\theta| \le \pi$ for every pair of endpoints.
