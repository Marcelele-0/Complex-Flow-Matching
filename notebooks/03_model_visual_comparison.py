...
cyl_ckpt = 'outputs/train/cylindrical_mini_bench/2026-08-29_18:26/checkpoints/checkpoint_epoch_25.pt'
...
print(f"  Cylindrical -> PSNR: {train_cyl['psnr']:.2f} dB | SSIM: {train_cyl['ssim']:.4f} | Phase Error: {train_cyl['phase_err']:.4f} rad")
print(f"  Euclidean   -> PSNR: {train_euc['psnr']:.2f} dB | SSIM: {train_euc['ssim']:.4f} | Phase Error: {train_euc['phase_err']:.4f} rad")
...
print(f"TEST SAMPLE (HELD-OUT):")
print(f"  Cylindrical -> PSNR: {test_cyl['psnr']:.2f} dB | SSIM: {test_cyl['ssim']:.4f} | Phase Error: {test_cyl['phase_err']:.4f} rad")
print(f"  Euclidean   -> PSNR: {test_euc['psnr']:.2f} dB | SSIM: {test_euc['ssim']:.4f} | Phase Error: {test_euc['phase_err']:.4f} rad")
...
axes[1].set_title('(b) Euclidean Flow ($R^2$)\nBoundary Seams & Phase Jump Artifacts', fontsize=14, fontweight='bold')
axes[1].axis('off')

im = axes[2].imshow(cyl_phase, cmap='twilight', vmin=-np.pi, vmax=np.pi)
axes[2].set_title('(c) Cylindrical Flow (Ours)\nNative $S^1$ Geodesic Preservation', fontsize=14, fontweight='bold')
axes[2].axis('off')
