...
print(
    f"CylindricalUNet Output Shape: {out_unet.shape} | Mean: {out_unet.mean().item():.4f}, Std: {out_unet.std().item():.4f}"
)

# 2. CylindricalUNetAttention
unet_attn = CylindricalUNetAttention(
    in_channels=3, out_channels=2, base_channels=8, channel_mults=[1, 2]
).to(device)
out_attn = unet_attn(x_2d, t)
print(
    f"CylindricalUNetAttention Output Shape: {out_attn.shape} | Mean: {out_attn.mean().item():.4f}, Std: {out_attn.std().item():.4f}"
)

# 3. CylindricalUNetCrossSlice (2.5D)
unet_25d = CylindricalUNetCrossSlice(
    in_channels=3, base_channels=8, channel_mults=[1, 2], attn_heads=2
).to(device)
out_25d = unet_25d(x_25d, t)
print(
    f"CylindricalUNetCrossSlice Output Shape: {out_25d.shape} | Mean: {out_25d.mean().item():.4f}, Std: {out_25d.std().item():.4f}"
)
