...
mkdir -p "$skills_dir"
ln -s "$PWD/docs/plgrid" "$skills_dir/plgrid-run"
```
...
`docs/plgrid/SKILL.md`.

## GitHub Copilot

Reference `docs/plgrid/SKILL.md` from `.github/copilot-instructions.md`.
...
Point the tool at `docs/plgrid/SKILL.md`. That file routes to the three references
